...
def query_9():
    ...
    cursor.close()
    root = tk.Tk()
    table = Table(root, headings=('Orchestra', 'Total Works played', 'Works below 50 yrs', 'Percentage'), rows=data)
    table.pack(expand=tk.YES, fill=tk.BOTH)
    root.mainloop()
...
def get_line_graph():
    ...
    sqlite_select_Query = ('with q2 as (\n'
                           'select orchestra,year, count(*) total_underage_events from (\n'
                           'select pa.orchestra, strftime(\'%Y\',date(pa.date_of_event, \'start of year\')) year,'
                           'pl.program, mc.composition_or_premiere_date,  '
                           'pa.date_of_event -mc.composition_or_premiere_date age\n'
                           'from programs_all pa, program_list pl, mappings_csv mc\n'
                           'where 1=1\n'
                           'and pa.event_id = pl.event_id\n'
                           'and pl.program=mc.link\n'
                           'and age <=50\n'
                           '--and date_of_event between \'1900-01-01\' and \'1920-01-01\'\n'
                           ')\n'
                           'group by orchestra,year\n'
                           '),\n'
                           'q3 as (\n'
                           'select orchestra , year, count(*) total_events from (\n'
                           'select strftime(\'%Y\',date(date_of_event, \'start of year\')) year ,pa.orchestra\n'
                           'from programs_all pa, program_list pl, mappings_csv mc\n'
                           'where 1=1\n'
                           'and pa.event_id = pl.event_id\n'
                           'and pl.program=mc.link\n'
                           ...
                           'select q3.orchestra ,\n'
                           ' \n'
                           'q3.year, q3.total_events, q2.total_underage_events\n'
                           ',  ifnull(round (((q2.total_underage_events*1.0) / q3.total_events ) *100,2),0) percent \n'
                           'from q3 left outer join q2 on q3.orchestra=q2.orchestra and q3.year = q2.year '
                           'order by q3.orchestra,q3.year')
    print(sqlite_select_Query)
    cursor.execute(sqlite_select_Query)
    x = cursor.fetchall()
    orch= []
    ...
    values = []
    i = 0
    orch_dict = {'Vienna Philharmonic': 1, 'Bamberg Symphony': 2,
                 'Philharmonia Hungarica': 3,
                 'Gewandhaus Leipzig': 4,
                 'Berlin Philharmonic': 5}
    x1, y1 = [], []
    x2, y2 = [], []
    x3, y3 = [], []
    x4, y4 = [], []
    ...
    for row in x:
        orch.append(int(orch_dict[row[0]]))
        data.append(row[1])
        values.append(row[4])
        if row[0]=='Vienna Philharmonic':
            ssg = int(row[1])
            print('x1.index(ssg)=', x1.index(ssg))
            print('x1=',x1)
            y1[x1.index(ssg)]=row[4]

            #y1.append(row[4])
        if row[0]=='Bamberg Symphony':
            x2.append(row[1])
            y2.append(row[4])
        if row[0]=='Philharmonia Hungarica':
            x3.append(row[1])
            y3.append(row[4])
        if row[0]=='Gewandhaus Leipzig':
            x4.append(row[1])
            y4.append(row[4])
        if row[0]=='Berlin Philharmonic':
            x5.append(row[1])
            y5.append(row[4])
    ...
    plt.plot(x5, y5 , label= 'Berlin Phil.')
    plt.plot(x1, y1 , label = 'Vienna PH')
    plt.plot(x2, y2 , label= 'Bamberg SYM')
    plt.plot(x3, y3 , label= 'Phil Hunagarica')
    plt.plot(x4, y4 , label= 'Gew. Leipzig')
...
if __name__ == '__main__':
    ...
    lbl = Label(window, text="Orchestra(s)+proportion of works 50 years "
                             "and younger \n at the date they were "
                             "performed within the chosen time period",
                font=('Arial Bold', 10))
    lbl.grid(column=0, row=0, columnspan=2)

    btn = Button(window, text='Fetch Query1', command=query_9)
    ...
    d1 = Entry(window)
    d2 = Entry(window)
    d1.grid(column=1, row=row_n, sticky='W')
    d2.grid(column=2, row=row_n, sticky='W')
    ...
    # btn = Button(window, text='Get graph', command=get_line_graph)
    # btn.grid(column=3, row=row_n)
    # btn = Button(window, text='Download as CSV', command=get_dates_csv)
    # btn.grid(column=4, row=row_n)
